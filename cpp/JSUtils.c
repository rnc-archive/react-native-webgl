...
void JSConsoleLog(JSContextRef ctx, const char *msg) {
  ...
  JSObjectRef console = (JSObjectRef) JSObjectGetPropertyNamed(ctx, global, "console");
  JSObjectRef log = (JSObjectRef) JSObjectGetPropertyNamed(ctx, console, "log");
  JSValueRef msgStr = JSValueMakeStringFromUTF8CString(ctx, msg);
...
JS_EXPORT size_t JSObjectGetTypedArrayByteLength(JSContextRef ctx, JSObjectRef object,
  ...
  JSConsoleLog(ctx, "JS: Tried to use non-existent TypedArray API");
  return 0;
}
